...
void testShapes() {
    ...
    @autoreleasepool {
        ...
        Rectangle *r, *r2;
        Square *s;
        NSString *filePath = @"archivedRect";
        BOOL ok;
        ...
        ok = [NSKeyedArchiver archiveRootObject:r toFile:filePath];
        if (!ok) {
            NSLog(@"Serialize Rect failed!");
            return;
        }
        r2 = [NSKeyedUnarchiver unarchiveObjectWithFile:filePath];
        NSLog(@"r2:");
        [r2 print];
    }
...
void testCopy() {
    ...
    @autoreleasepool {
        ...
        NSMutableString *muStr;
        arr1 = [NSMutableArray arrayWithObjects:
        ...
        muStr = [NSMutableString stringWithString:arr2[0]];
        
        [muStr appendString:@"one"];
        
        [arr1 replaceObjectAtIndex:[@0 integerValue] withObject:muStr];
        
        NSLog(@"arr1: ");
        for (NSMutableString *e in arr1) {
            NSLog(@"%@", e);
        }
        NSLog(@"arr2: ");
        for (NSMutableString *e in arr2) {
            NSLog(@"%@", e);
        }
    }
